...
if not os.path.exists('./charts/rq2B_techniques'):
    os.mkdir('./charts/rq2B_techniques')

mod_paths = ['aaa', 'bbb', 'ccc', 'ddd', 'eee', 'fff', 'ggg']

clf_paths = ['DT', 'FF', 'KNN', 'NB', 'NN', 'RF', 'SVM']
tch_paths = ['SMOTE', 'SMOTE+AE', 'SMOTE+KMEANS', 'SMOTE+PCA']
...
for mod in mod_paths:
    ...
    for met in metrics:
        ...
        base_df = met_df[met_df['technique'] == 'smote'].copy(deep=True)
        ae_df = met_df[met_df['technique'] == 'smote+ae'].copy(deep=True)
        km_df = met_df[met_df['technique'] == 'smote+kmeans'].copy(deep=True)
        pca_df = met_df[met_df['technique'] == 'smote+pca'].copy(deep=True)
        ...
        ae_plot = sns.barplot(x='clf+atbt', y='score', data=ae_df, color='#3cb371')
        ae_plot.set_title(f'{mod.upper()} : {met.upper()} \n Effect of Autoencoding (SMOTE)', fontsize=15)
        ...
        ae_plot.figure.savefig(f'./charts/rq2B_techniques/{mod.lower()}_{met}_ae.png')

        plt.clf()

        sns.set(style='darkgrid')
        km_plot = sns.barplot(x='clf+atbt', y='score', data=km_df, color='#3cb371')
        km_plot.set_title(f'{mod.upper()} : {met.upper()} \n Effect of K-Means Clustering (SMOTE)', fontsize=15)
        ...
        ae_plot.figure.savefig(f'./charts/rq2B_techniques/{mod.lower()}_{met}_km.png')

        plt.clf()

        sns.set(style='darkgrid')
        pca_plot = sns.barplot(x='clf+atbt', y='score', data=pca_df, color='#3cb371')
        pca_plot.set_title(f'{mod.upper()} : {met.upper()} \n Effect of PCA Feature Extraction (SMOTE)', fontsize=15)
        ...
        ae_plot.figure.savefig(f'./charts/rq2B_techniques/{mod.lower()}_{met}_pca.png')
